...
如果你（们）只是想单纯地抱住一个敌人，并不想举起来，你（们）可以选择擒抱他。

擒抱与举起角色的流程相同，区别在于此时敌人等价的物品搬运成功难度值是举起的一半。若擒抱成功，你（们）会进入<a href="../../../data/status/normal/#束缚" target="_blank">“束缚”</a>状态，被擒抱者视为被束缚者。特别地，你（们）无法在此状态下执行束缚者的行动1和行动2，且被擒抱者若受到攻击，所有擒抱者也需要进行如同解除手段2中的解除流程。
