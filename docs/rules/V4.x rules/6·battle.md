...
!!! question "HP归零了怎么办"
    如果对手没打算下死手，且你的HP并没有`低于你体质负值的一半`，你只是陷入<a href="../../data/status/normal/#濒死" target="_blank">“濒死”</a>状态与<a href="../../data/status/normal/#倒地" target="_blank">“倒地”</a>状态，HP上限固定到0，等人来营救即可。要注意的是，此时你会失去你的回合：同一行动轮内到你的回合，若你濒死，会延后回合到本行动轮的末尾，如果这一行动轮结束时你仍然没有解除<a href="../../data/status/normal/#濒死" target="_blank">“濒死”</a>状态，那你就会丢失你这一行动轮的回合。

    如果对手下了死手，或者你的HP`低于你体质负值的一半`，那么就会在<a href="../../data/status/normal/#濒死" target="_blank">“濒死”</a>和<a href="../../data/status/normal/#倒地" target="_blank">“倒地”</a>的基础上，陷入<a href="../../data/status/normal/#致命伤" target="_blank">“致命伤”</a>状态：每行动轮（10秒）你需要过一个“抵抗”技能的检定，如果失败，HP再-5，直到你的HP`低于你体质负值`时，你正式死亡。你的灵魂会从肉体中飞出，种族转变为“幽灵”，失去所有装备，如果在场没有巫妖指引，则默认前往魔界。（如果本身就在魔界则会不受控制四散而逃，即是所谓的游魂）

    如何在上述状态下自救，并恢复到继续战斗的状态？第一步，你需要把你的HP修正到0点，例如通过喂药等手段，但此时的HP的上限是0，不会超过0。第二步，使用能够解除<a href="../../data/status/normal/#濒死" target="_blank">“濒死”</a>与<a href="../../data/status/normal/#致命伤" target="_blank">“致命伤”</a>状态的药品或能力，这么做会把你的HP固定到1点，此时你就能恢复意识，拥有自己的回合了。第三步，到你的回合时，消耗2行动点解除<a href="../../data/status/normal/#倒地" target="_blank">“倒地”</a>状态，然后是回血还是进攻就随你的便了。
...
!!! success "行动点（AP）"
    ...
    你的每个回合开始时，回复与最大值等值的基础行动点（默认6点）与敏捷提供的行动点（默认敏捷/10（向下取整））。特殊情况下，假如你的行动点被某些行为扣除到负数，则在回复阶段完毕后，在敏捷提供的行动点处扣除这些行动点。

你的回合中，如果有剩余的非基础行动点没有用完，则可以保存并累加到下一行动轮，或是在敌方行动轮使用某些被动技能，但这种保存只能持续一行动轮。如果有剩余基础行动点没有用完，则会直接消失。
...
飞行的过程中，每纵向向上移动1格（1米），消耗1.0倍行动点；每纵向向下，或横向移动1格，消耗0.5倍行动点。在计算空间斜向移动距离时，先计算垂直位移（剔除斜向要素），再计算水平位移（可包含斜向要素），最后将其加总即可。

在空中结束你的回合是一件危险的事。如果你要这么做，将会遭受以下惩罚：
* 你每距离地面1格，你就要额外花费3点SP来维持你身体的平衡。如果你的SP不足，则在你结束回合的瞬间，你立刻如自由落体般坠落到地面
* 你身上的装备将会限制你在空中行动的自由度。若你的整体护甲类型为轻甲/中甲/重甲，在下个行动轮你的回合的回复步骤中，你少回复2/4/6点行动点（如果你骑在坐骑上，则计算整体护甲时，将你坐骑的护甲类型也算上）
* 你每在空中结束一次你的回合，你在空中的所有检定额外获得一枚惩罚骰，直到你在地面结束你的回合，此累计清空
...
* 每次回合开始时，要进行一次溺水判定，具体来说就是执行一次“水性”（与体质挂钩）技能当中的屏息行动（不消耗行动点数），如果过了则无事发生，如果没过，则叠加一层<a href="../../data/status/mark/#溺水" target="_blank">“溺水”</a>
* 当你在水下行动陷入<a href="../Extra/BulletTime" target="_blank">破绽</a>，或者单次受到的伤害超过最大生命值的10％时，你也要立刻进行一次溺水判定
...
!!! success "行动轮数计算规则"
    对于玩家而言，某一状态在经过一次所有人的回复步骤之后算作经过一行动轮，这与仪式之法的准备时间一致。
...
!!! success "进攻阈值衰减公式"
    ...
    特别地，使用特殊战技和魔法不会算上阈值衰减。若拥有减少衰减数值的能力，最多将衰减数值减少到原值的一半。

    进攻阈值衰减不会把进攻阈值扣到太低，最低衰减到原值的一半就不会再继续衰减了。
...
!!! success "防守阈值衰减公式"
    ...
    特别地，使用拼刀用的是进攻阈值衰减规则，而且拼刀时双方的进攻阈值衰减速度均减半。若拥有减少衰减数值的能力，最多将衰减数值减少到原值的一半。

    防守阈值衰减不会把防守阈值扣到太低，最低衰减到原值的一半就不会再继续衰减了。
...
如果你使用的武器或使用的能力中写明“造成的伤害为非致命伤害”，则你所有的进攻行为均视为没有下死手。

其他情况下，你所有的进攻行为默认视为有下死手。如果你不想下死手，你需要在本次进攻前宣言此次进攻不下死手。为了避免麻烦，你可以在整场战斗开始时宣言，你在本场战斗的所有进攻的默认下死手情况（不宣言则视为默认下死手）。
...
!!! example "举例：普通战斗"
    ...
    另外，骨灰剩余了`1`点额外的行动点，将其保存了下来，供下行动轮使用。

!!! example "举例：拼刀（例子中忽视了防具减伤效果与阈值衰减）"
    骨剑和阿尔西斯展开了宿命的对决。
    
    两人本行动轮内剩余行动点均只剩`3`点。两人展开了一对一的拼刀，并由骨剑发起先攻，占据主动。
...
* 任何角色可以花费1行动点在半身掩体旁边蹲下，将其视为全身掩体，此时无法移动，可以再花费1行动点起身
...
[![pE0ro36.png](https://s21.ax1x.com/2025/03/22/pE0ro36.png)](https://imgse.com/i/pE0ro36)
...
[![pE0rTgK.png](https://s21.ax1x.com/2025/03/22/pE0rTgK.png)](https://imgse.com/i/pE0rTgK)
