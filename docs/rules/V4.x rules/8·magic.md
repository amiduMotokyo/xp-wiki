...
2.1.1：如果这个角色对你释放了带有攻击伤害的魔法，你需要用你的“魔法抗性”检定去对抗他的魔法检定，决定你受到的法术伤害，得到差值因子x。假如这个魔法附带特殊效果，且你在魔法抗性的对抗中取得成功，可以免疫这个法术附带的特殊效果，若失败，你还需要进行一次普通的抵抗检定，来判断你会不会因此中异常状态。随后，你需要（也只能）进行一次普通的闪避检定（计算防守衰减），难度为之前得到的差值因子x乘上5，如果成功则受到的法术伤害减半；如果闪避失败，则受到全额的法术伤害（无视你装备的护甲值和减伤）。
...
“xx系精通”特殊技艺无法通过`分配经历点获得的经验值`的方式来增长熟练度。每当你使用本系内的某一种魔法，检定失败/成功/大成功大失败时，对应“xx系精通”特殊技艺就会获得2/3/6点经验值。
...
2|15|连发消耗行动力减半（n连发所消耗的行动力就是1+n）|2
3|45|允许跨魔法连发（换魔法时须重新检定）|3
4|105|连发消耗SP减半（首发SP全额，之后连发SP半额）|3
5|165|每行动轮允许无损耗获得1连发（不记入最大单次连发次数内）|3
...
!!! example "举例：使用漩涡之法流派魔法（部位检定省略）（SP消耗省略）"
    ...
    由于冰雹术附加状态“晕眩”的存在，帕吉尔必须使用与体质挂钩的技能“抵抗”来避免晕眩。
    
    帕吉尔的体质属性为`45`，抵抗技能熟练度为`21`，他投出了`34`，`34<45+21`，于是成功抵抗住了晕眩状态的完整附加，只受到弱化版的影响。然而，3连发意味着他还要再重复执行2次这样的抵抗操作。
...
* 心神之法流派魔法（下称心神之法）不需要消耗SP就能进行使用，但必须确保你的剩余SP在5点及以上，否则无法使用
* 每种心神之法默认一行动轮只能使用一次
* 你可以通过“过载”来获得额外的效果加成效果，这些效果写在了下文“成长”栏目的表格当中。默认情况下，每对一个魔法进行一重过载，额外消耗10点SP和1点行动力，对于一次过载，每一种过载效果最多叠2次。对于一些效果不容易量化，只给目标上buff的心神之法，你可以通过过载2次“魔法效果+0.5倍”来达到“给同一人施加2次buff”的效果。
* 你的当前状态决定了你当前过载重数的上限：你剩余SP/10（向下取整）代表你目前最多能对单个魔法进行几重过载，
* 你的整体能力决定了你在1行动轮内总计进行多少次过载不会疲劳：你的除魔力外其他属性总和/50（四舍五入）决定你在1行动轮内最多使用几次过载不会疲劳（假设你先释放A魔法并施加了2重过载，然后释放了B魔法并施加了1重过载，那么你在1行动轮内就是使用了2+1=3次过载）
* 如果超过了过载次数限额，使用完魔法后会陷入<a href="../../data/status/normal/#疲劳" target="_blank">“疲劳”</a>状态，所有检定要获得一枚惩罚骰，恢复这一状态所需的行动轮数取决于你的超额量（例如你本行动轮最多使用3次过载不会疲劳，而你使用了4次过载，则疲劳就只持续1回合）
* 本行动轮每对某一魔法进行一重过载，且消耗了SP，则该魔法获得的经验值额外+2。
...
:--:|:--:|:--:|:--:
1|0|不允许|抵消检定值超过阈值的惩罚、魔法效果+0.5倍（若同时拥有增加作用目标数的过载效果，则只能选取一个目标增加魔法效果）
2|10|每行动轮允许无损耗进行1次过载|施法范围+3
3|30|每行动轮允许无损耗进行1次过载|作用目标数+1（不可对同一人重复使用）
4|70|每行动轮允许无损耗进行2次过载|增加一级检定成果（但无法增加到大成功）
5|110|每行动轮允许无损耗进行2次过载|允许缝合另一心神之法流派魔法的效果

注：无损耗意味着，本次过载不消耗额外SP与行动点，不计入疲劳限额，不计入过载重数上限。增加一级检定成果意味着：成功→困难成功（阈值/2），困难成功→极难成功（阈值/5）（具体检定值固定为该检定成果的最大值）。
...
!!! example "举例：使用心神之法流派魔法"
    ...
    凯南的SP量为`55`，每行动轮最多能使用`3`重过载不会疲劳。
    
    凯南的魔力为`55`，治疗术熟练度为`18`，他使用了治疗术，骰出了`75`，不幸的是，这次使用的治疗术疗效必须打折扣了。
    
    于是他使用了一重过载，这重过载的效果是将检定值超过阈值的那部分惩罚抵消。
    
    为了能一次性治疗到多名患者，他使用了第二重和第三重过载，效果都是将治疗术的目标`+1`，结果就是`+2`。由于流派等级`3`级的效果，凯南还能再免费获得一重过载，于是他选择了将针对某重伤患者的魔法效果+0.5倍。
    
    这样一来，凯南在不会陷入疲劳的状态下使用了`4`重过载的治疗术，效果是使2名患者受到1倍疗效的治疗术效果，使1名重伤患者受到1.5倍疗效的治疗术效果。。
...
仪式之法流派的魔法，需要消耗一定的素材才能使用，通常和信仰和空间移动有关，例如圣光术，传送术，天狗国的阴阳术等等。这类魔法的特点是学习成本高，烧钱，准备时间长，但能够一次性产生诸多持续性的buff效果。

#### 使用方法

若无特殊说明，仪式之法的击发类型均为定点型。

* 仪式之法流派魔法（下称仪式之法）需要同时消耗SP和素材才能进行使用，通常一个完整的仪式之法会给出总共需要多少素材和消耗多少SP。素材分为一次性素材和可重复素材，一次性素材使用1次就会消失，而可重复素材使用多次才会消失，甚至有的素材可以无限次被使用
* 仪式之法需要准备一段时间后才能使用。当你选定你要释放哪些仪式之法后，立刻消耗对应的素材和SP，然后进入准备仪式，每选择一个仪式之法，默认情况下消耗行动点+2
* 如果某仪式之法准备时间内包含“半行动轮”，则统计战场上所有人的人数，将其减半后向上取整，换算成对应的回合数即可。另外，只要仪式之法原始存在准备时间，并非立刻生效，那么通过各种手段减少准备时间，最多只能将准备时间减少到2回合，如果2回合多于半行动轮则取半行动轮
* 对于同一种，默认情况下是可以在1行动轮内同时准备多个的，也可以同时对同一个目标释放。但有相当一部分类型的仪式之法不能这么做，这些仪式之法会在开头就写明，请仔细辨明
* 进入准备仪式的那一刻，由于操作复杂，此时会露出一次<a href="../Extra/BulletTime" target="_blank">破绽</a>（如果是立刻生效的则不会露出破绽）。准备期间只能进行移动，无法执行其他行动
* 当某个仪式之法准备时间结束后，你可以在施法范围内选择释放的目标，立刻将魔法效果附加在目标身上，无需任何检定
* 每种仪式之法都有可以被打断的“弱点部位”。如果在准备期间，施法者被进攻到了某仪式之法的“弱点部位”且受到了伤害，则要进行施法者的“魔法抗性”的检定，难度为之前进攻的差值因子×5，若没过，该仪式之法被打断，立刻失效；如果在准备完成后，受术者被进攻到了某仪式之法的“弱点部位”且受到了伤害，则还是需要施法者进行相同的操作。若一个“弱点部位”对应了多个不同的仪式之法，则一次打断只能失效一个仪式之法
* 仪式之法提供的buff效果，基本上只要不被打断，就能持续整场战斗。不过生效期间目标必须保持在施法者的感知范围或施法范围内，施法者死亡或是目标离开施法者的感知范围或施法范围，仪式之法就会立刻失效
* 在战斗外，仪式之法只能持续1小时，时间结束后需要重新消耗素材“续杯”才能继续生效
* 只有存在对应信仰的角色才能尝试对某仪式之法进行斗法，双方以魔法感知进行对抗
...
:--:|:--:|:--:
1|0|每场战斗最多2次，在你释放的某仪式之法效果结束时，回收该仪式之法消耗的素材
2|10|在一次仪式中，准备第二个及第二个以上的仪式之法只消耗1行动点
3|30|所有仪式之法消耗SP减半
4|70|所有仪式之法可选择让准备时间减少半行动轮
5|110|进入准备仪式时不再露出破绽

#### 举例

!!! example "举例：使用仪式之法流派魔法（SP消耗省略）（检定部分省略）"
    流派等级为`2`的斋藤忍掌握着八咫鸟相关的阴阳术。
    
    在她所了解到的阴阳术当中，“寻路视界”需要消耗`1`张斋藤神签，准备时间为半行动轮（战场上总共有6人，因此被换算成3个回合），弱点在施法对象的眼部（头部）；“群鸦之怒”需要消耗`1`张斋藤神签和`1`根鸦羽，准备时间为半行动轮，弱点在施法者的左右手。
    
    在一次报社取材的过程中，神经质的社长又说周围可能有埋伏了，要求阿忍帮忙探探路。
    
    无奈之下，阿忍消耗`2+1+1+1=5`点行动力，先消耗了`3`张斋藤神签，打算给自己的`3`位队友加上“寻路视界”，同时又消耗了`1`张斋藤神签和`1`根鸦羽同时释放“群鸦之怒”，仪式准备开始，准备期间阿忍只能进行移动，无法再进行其他行动。
    
    不巧的是，周围潜伏的伏兵率先一步跳了出来攻击了阿忍的左手，企图通过攻击“群鸦之怒”的弱点来打断阿忍的仪式。
    
    阿忍必须用魔法抗性来对抗，好消息是对抗成功了，“群鸦之怒”的准备并没有被打断，但阿忍还是确确实实地承受了来自伏兵的伤害。`3`个回合之后，仪式结束，“寻路视界”和“群鸦之怒”的效果同时生效，我方全员的侦查检定获得一枚奖励骰，持续整场战斗。
    
    这时，伏兵攻击了元清的头部，企图通过攻击“寻路视界”的弱点来打断正面buff的效果。
    
    这时，元清采取“防御”来应对本次攻击。然而即使他防御成功，也只能减免75％的伤害，在这之后仍然需要阿忍进行魔法抗性检定来判断该魔法效果是否被打断。然而，奇迹出现了：元清的这次检定结果为“大成功”，触发了“完美防御”，免除了所有伤害，因此也就不需要阿忍进行打断判定了。
...
### <h id="通过魔法书学习魔法">通过魔法书学习魔法</h>

你可以在长休中花费2点长休行动点，进行灵知检定，学会一个初级/中级/上级魔法需要成功1/2/3次。此举获得经验值+1倍。
