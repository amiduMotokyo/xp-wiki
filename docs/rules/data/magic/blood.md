...
* 初始：战斗时对自身施法范围/2（最低为2）的单位造成伤害时，获得该伤害值1/4（向上取整）的护盾屏障。若为血族则可选择改为恢复生命值。
...
* 初始：获得（检定差值/10四舍五入）护盾屏障，可额外消耗HP生成（HP×1.5）护盾屏障，持续一回合，可每回合付出3行动点延长。优先于全部伤害计算扣除，不计算任何护甲与减伤。且护盾屏障生命值不可超过自身生命值一半。
* 10级进阶：持续时长+1回合，同时额外消耗可使用血袋，每份提供6点护盾屏障。
