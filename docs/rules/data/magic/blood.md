...
* 初始：目标获得（检定差值/5四舍五入）+（额外消耗HP×1.5）的点数，该点数可用于等价替换本回合每次攻击的进攻衰减，代价是这种进攻需要额外扣除5点HP
* 10级进阶：升级为（检定差值/4四舍五入）+（额外消耗HP×2）
* 20级进阶：升级为（检定差值/3四舍五入）+（额外消耗HP×2），进攻只需要额外扣除3点HP
...
* 20级进阶：升级为使用这种武器进攻若成功，敌人抵抗判2层流血。
...
* 初始：自身每回合获得（额外消耗hp/5）点额外行动点（最多6点），代价为每回合要消耗（额外消耗hp/2）点HP；开启后，每回合开始需要再过一次本魔法检定，若没过，则中断本魔法效果。
* 10级进阶：现在每回合只需要消耗（额外消耗hp/3）点HP
* 20级进阶：现在每回合只需要消耗（额外消耗hp/4）点HP
* 30级进阶：现在每回合只需要消耗（额外消耗hp/5）点HP
* 30级名称改变：猩红狂热
...
* 初始：自身使用鲜血魔法最终的HP消耗减少（检定差值/10四舍五入）点（不影响最终收益），且使用瓶装血/血袋也可以提供一半的额外施法消耗增益，持续一回合。每场战斗最多使用一次，不可过载。
* 10级进阶：现在使用瓶装血/血袋可以提供完整的额外施法消耗增益
* 20级进阶：现在使用鲜血魔法最终的HP消耗减少（检定差值/8四舍五入）点（不影响最终收益）
* 30级进阶：持续时间延长一回合
* 30级名称改变：长效猩红飞升
