...
!!! warning "塔罗类魔法与其他魔法的互斥性"
    过度干涉未来是不会有好结果的。当你一场战斗中同时使用了塔罗系魔法的上级魔法与其他魔法的上级魔法时，你会遭受命运的惩罚：行动轮结束时强制生命值减少至1并进入“濒死”与“致命伤”。

    同时干涉他人命运也是会遭到一些神的不满，使用其信仰类魔法时会有不同的惩罚（解释权在对应作者手中，建议不用）。

### 理解并操纵命运的能力

塔罗系法术本身的效果相比一般的魔法较差，且无法过载。但每次使用或获得“命运之牌”时会让使用者更能理解塔罗系魔法的奥秘，战斗结束时每张使用过的“命运之牌”会提供2点可自由分配于塔罗系魔法的经验值，未使用的卡牌则只获得1点。
...
你所能保有的塔罗牌数量是有限的，你每学习1个塔罗系魔法会增加1个保留的数量，行动轮结束时选择丢弃超过保有上限的塔罗牌。每行动轮开始时，进行抽牌（指令为。rd22）来获得塔罗牌，一次抽取到保留数量的上限。（多次抽取的指令为。r个数d22）
...
5皇帝|使目标下一个对抗成功时的因子倍率+0.5倍|使目标下一个对抗成功时的因子倍率-0.5倍（最低0.5倍）
6教皇|使目标下一个说服话术获得1个奖励骰|使目标下一次被动心理学鉴定获得1个惩罚骰
7恋人|使本次魔法施加的阈值与鉴定奖励效果+1倍|使本次魔法施加的阈值与鉴定惩罚+1倍
8战车|使目标本行动轮每次成功的对抗恢复5SP（最多15）|使目标本行动轮每次失败的对抗减少5SP（最多15）
9力量|使目标下一个对抗获得阈值+15的奖励|使目标下一个对抗获得阈值-15的奖惩罚
10隐者|若目标有剩余AP则使目标本行动轮获得阈值+5奖励|若目标无剩余AP则使目标本行动轮获得阈值-5惩罚
11命运之轮|目标本行动轮进入子弹时间时获得-5鉴定奖励|目标本行动轮进入子弹时间时获得+5鉴定惩罚
12正义|目标本行动轮进行进攻/防守鉴定时，使下一次不同类型的鉴定获得1个奖励骰|目标本行动轮进行进攻/防守鉴定时，使下一次相同类型的鉴定获得1个惩罚骰
13倒吊者|目标本行动轮下次受到伤害时恢复10%最大HP|目标本行动轮下次受到伤害时损失5%最大HP
14死亡|若目标生命值不足一半则下次受到的伤害总伤倍率+1|目标次受到伤害后不会“濒死”
15节制|自己同时获得目标受到魔法的效果，且获得一个相同的buff|额外选择一个目标受到与本次魔法的目标相同效果，且获得一个相同的buff
...
* 效果：使目标当前其他全部增益效果延长1行动轮/使目标当前其他全部减益效果延长1行动轮
* 解除手段1：1行动轮后自动解除。
* 弱化版本：无，直接解除。
* 词条：内生型
...
* 初始：给予友军1张塔罗牌，永久获得该牌，每行动轮开始时触发该效果。但自身无法再使用该塔罗牌。最多给予1张。
...
* 30级进阶：拥有“世界”时每行动轮可免疫一个受到的异常效果。且“世界”使用后不会消失可重复使用。
