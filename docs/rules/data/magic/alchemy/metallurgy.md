...
* 初始：向某物品注入（物品生命值×10）g塑型土，赋予其移动能力，每行动轮可以操控其移动6格，但无法移出施法者的感知范围；施法者可以随时取消该魔法；当物品被攻击时，施法者可以用活物术的阈值来替物品进行防守检定
...
特别地，如果你炼成的是飞镖，则一次检定同时享受炼成武器和炼成弹药的所有额外投入效果和检定增益。
...
使用物品炼成炼出的防具都是1级中型防具，不过，物品炼成检定每（差值/10）向下取整能额外提供1点护甲。
