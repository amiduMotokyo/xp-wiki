...
自然，有着“聚魂术”也会有“散魂术”，这个最初由某位巫妖通过将“聚魂术”反向操作来给竞争对手下绊子的法术目前似乎已经从巫妖的圈子中流传了出去，成为了一些所谓“巫妖猎人”的王牌技能，请切勿小心。

亡灵法术的适应度效果见下表

适应度等级|对应场景|影响
:--:|:--:|:--:
-2|圣堂周边|亡灵召唤物武器威力-2，伤害倍率-0.2，无法使用特殊能力；初级亡灵法术获得一枚惩罚骰且施放时需额外消耗4SP
-1|城镇|亡灵生物武器威力-1，伤害倍率-0.2；初级亡灵法术获得一枚惩罚骰
0|郊野|无效果，大部分场景下的默认状态
1|墓园|亡灵生物武器威力+1，获得2点护甲穿透；初级亡灵法术获得一枚奖励骰
2|战场遗迹|亡灵生物武器威力+2，获得2点护甲穿透，因子倍率+1；初级亡灵法术获得一枚奖励骰，检定值-4
...
* 亡灵族，免疫带“活体”词条的异常状态
* 他人对你进行远程攻击时，失去所有奖励骰
* 远程进攻时消耗10点SP发动，当次进攻阈值+8，若成功，额外造成4点固伤且目标陷入破绽
...
* 20级进阶：僵尸护甲值+1，近战攻击成功时额外造成2点固伤
* 30级进阶：僵尸武器威力增加召唤者经历点/10向下取整，SP+10，特殊能力2每回合使用次数+1
...
* 护甲3减伤30%额外生命9
* 亡灵族
* 特殊能力1：近战进攻时消耗5点SP发动，当次进攻阈值+8，若成功则额外造成2点固伤
* 特殊能力2：消耗10点SP与进攻次数，死死咬住目标距离1的角色，若成功则造成4点固伤，双方同时进入<a href="../../../status/normal/#束缚" target="_blank">“束缚”</a>，但无法执行束缚行动1，目标每次挣脱失败受到2点固伤<br>该能力每回合可使用1次，所有僵尸共享使用次数
...
* 初始：炸毁目标格中自己控制下的所有亡灵单位，每炸毁一个便对其半径2（大型单位则为4）的所有单位造成一次威力相当于所炸毁单位武器威力1.5倍的物理伤害，那之后所炸毁单位所占用的召唤位直到下个自己的回合结束无法使用
...
* 20级进阶：被炸毁的单位不再锁定召唤位
* 30级进阶：被炸毁的单位剩余HP每有5点，法术威力便再+1，被炸毁的单位剩余SP每有5点，便额外造成2点SP减损
...
* 30级进阶：差值每有20，便选择一项未选过的：<br>·额外令适应度上升或下降一级<br>·适应度变动额外持续一轮<br>·自己控制下的亡灵召唤物本回合进攻检定值-8<br>·不处于自己控制下的亡灵召唤物本回合防守检定值+8
...
* 20级进阶：威力+2，每次使用时仅一次，可以额外消耗1层充能，在伤害结算后回复相当于差值因子两倍的SP
