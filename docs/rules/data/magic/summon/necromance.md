...
* 每种召唤魔法召唤出的亡灵生物组同一时间只能保持3份
* 使用相同的召唤魔法（不消耗SP）可以将自己的亡灵生物组消除1份，回收算力
* 亡灵生物的所有技能阈值都是`当地亡灵法术适应度`加上`召唤者对应召唤魔法的熟练度`再加10
...
* 没有护甲
* 敌方对自己进行远程攻击时获得一枚惩罚骰
* 初级：HP+10，SP+8，所有检定阈值+10
* 中级：HP+10，SP+8，所有检定阈值+10
* 上级：HP+10，SP+8，所有检定阈值+10
...
* 初级：HP+12，SP+8，所有检定阈值+10
* 中级：HP+12，SP+8，所有检定阈值+10
* 上级：HP+12，SP+8，所有检定阈值+10
...
* 中级：每回合获得8点行动点，获得初级等级的所有战术，获得20级武器通用技能
...
* 初始：允许将游魂魂附到指定载体当中，使之成为精英单位，解放施法者的算力，战斗结束后失效；允许将自愿的幽灵种族角色魂附到指定载体当中，使之成为NPC或PC，这种魂附方式拥有2天的冷却时间
* 10级进阶：冷却时间降低为1天
* 20级进阶：冷却时间降低为一次长休
* 30级进阶：现在没有冷却时间
