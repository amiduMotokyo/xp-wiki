...
* 累加效果：每一层点睛印记使所有<a href="../mark/#范本" target="_blank">范本</a>防守检定获得检定值+4惩罚。
* 解除手段：一行动轮后自动清空。
* 词条：外生型
...
* 累加效果：每两层“忽悠印记”使自身在下个回复阶段回复的基础行动点-1，最多叠加6层。
* 累加质变：累积层数4层时，话术施加者可选择清除4层“忽悠印记”，使目标强制附加完整版的<a href="../normal/#迟疑" target="_blank">“迟疑”</a>状态。
* 解除手段：每行动轮减少2层。
* 词条：话术
...
* 累加效果：每一层“坚定印记”可以在对抗能够附加话术效果能力时，使检定阈值获得+3奖励，最多叠加8层。
* 累加质变1：可随时选择清除2层“坚定印记”，弱化自身的某话术效果，或是将一种话术印记层数清除一半（至少清除1层）。
* 累加质变2：累积层数4层时，在对抗能够附加话术效果能力时，使检定额外获得一枚奖励骰。
* 解除手段1：与<a href="../mark/#劝服印记" target="_blank">“劝服印记”</a>相互抵消。
* 解除手段2：战斗结束后自动解除。
* 词条：话术

### <h id="剑风印记">剑风印记</h>

* 累加效果：使用剑系武器允许使用且消耗不带X的特殊战技时，可消耗1层该印记代替当次行动点与SP消耗，最多叠加3层。
* 解除手段：每次使用后解除一层。
* 词条：内生型

### <h id="解剖印记">解剖印记</h>

* 累加效果：闪避每次衰减，便使防御衰减那个数值的（层数/2）倍，至多叠加2层
* 解除手段：1行动轮后自动清空
* 弱化版本：叠加层数减半（向下取整）。
* 词条：外生型
...
* 累加效果：每一层“惊吓印记”使自身的所有检定值获得+1的惩罚，最多叠加6层。
* 累加质变：累积层数4层时，话术施加者可选择清除4层“惊吓印记”，使目标强制附加完整版的<a href="../normal/#振恐" target="_blank">“振恐”</a>状态。
* 解除手段：每行动轮减少2层。
* 词条：话术
...
* 累加效果：每有一层磐石赐福，护甲值+1，获得3点该防具的额外生命，最多叠3层
...
* 累加效果：层数存在时，你的“潜行”或“隐形”效果失效，也不能被他人视为“潜行”或“隐形”状态，最多叠加2层。
* 特殊效果：只要存在过该印记，即使脱离战斗，气息也会在1天内使其他人对你进行追踪时获得奖励（由主持人决定）。
* 解除手段：每行动轮减少1层。
* 词条：外生型

### <h id="劝服印记">劝服印记</h>

* 累加效果：每一层“劝服印记”可以在对抗能够附加话术效果能力时，使检定阈值获得-3惩罚，最多叠加8层。
* 累加质变1：累积层数4层时，在对抗能够附加话术效果能力时，使检定额外获得一枚惩罚骰。
* 累加质变2：累积层数8层时，话术施加者可选择清除8层“劝服印记”，使目标强制附加完整版的<a href="../normal/#劝服" target="_blank">“劝服”</a>状态（若目标为boss，则改为弱化版）。
* 解除手段1：与<a href="../mark/#坚定印记" target="_blank">“坚定印记”</a>相互抵消。
* 解除手段2：战斗结束后自动解除。
* 词条：话术
...
* 累加效果：每有一层“水之领域”，角色周围一圈视为<a href="../terrain/#川流地形（深水）" target="_blank">“川流地形（深水）”</a>，每叠一层加一圈，最多叠加2层。你可以在叠加的时候选择是否让角色所处的那一格也视为<a href="../terrain/#川流地形（深水）" target="_blank">“川流地形（深水）”</a>
* 解除手段1：准备时，进攻施法者躯干。
...
* 累加效果1：对当前主职所在社群为“红蔷薇会”的角色进行敌意行动时，失去层数×3的SP，那之后，若SP小于上限的一半，则那次敌意行动获得一枚惩罚骰，最多叠加6层。
...
* 累加效果：任意检定前可选择消耗最多一层，同时消耗2SP，获得一层战术奖励，视为使用一次战术，最多叠加3层。
* 解除手段：每次生效后，层数-1。
* 词条：内生型
