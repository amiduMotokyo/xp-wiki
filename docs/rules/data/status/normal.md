...
* 效果：HP无法超过HP上限的一半，每回合获得额外2点AP，敌意行为阈值+4
* 解除手段：战斗结束后解除
* 弱化版本：无
* 词条：无
...
* 效果：浮空状态是一种特殊的搬运状态，是否附加浮空状态的判法与<a href="../../../V4.x rules/Extra/Weight" target="_blank">重量规则</a>中的搬运相同，区别在于与重物搬运成功难度值比较时，浮空术检定提供的差值+0.5倍，且不允许联合浮空<br>当施加者成功使一个目标附加“浮空”状态后，施加者可以花费自己的行动点操控它进行移动，东南西北1格与向下1格都需要花费1行动点，而向上1个需要花费1.5行动点。特别地，如果施加者对某一件装备附加了“浮空”状态，则可以让它的力量阈值减少浮空术检定的差值<br>施加者可以花费2行动点，对目标进行类似于<a href="../../../V4.x rules/Extra/Weight" target="_blank">重量规则</a>中的抛掷行动，区别在于将检定中的“搬运”改为施加者的“浮空术”<br>在施加者的回合结束时，若目标仍然在空中，则强制让目标自由落体落地<br>默认情况下，施加者只能同时指定1个目标附加“浮空”状态<br>若目标为角色，他在自己的回合里仍然可以自由行动
* 解除手段1：1行动轮后自动解除。
* 解除手段2：施加者在自己的回合里解除目标的“浮空”状态（无消耗）
* 弱化版本：施加者最多消耗3行动点来操控目标，且无法进行抛掷行动。
* 词条：内生型
...
* 效果：处于自身施法范围内的所有友方角色（包括自己），每次成功的对抗都可以对目标敌人附加一层<a href="../mark/#集群锁定" target="_blank">“集群锁定”</a>。
* 解除手段：进攻施法者左右手。
* 弱化版本：本效果起作用的施法范围减半。
* 词条：信仰
...
* 效果：叠加1层<a href="../mark/#解剖印记" target="_blank">“解剖印记”</a>。
* 解除手段：1行动轮后自动清空
* 弱化版本：叠加层数减半（向下取整）。
* 词条：外生型
...
* 效果：目标每行动轮开始时进行阈值为50的检定，若失败，目标这行动轮的下一个成功的检定自动失败。
* 解除手段1：准备时，进攻施法者躯干。
* 解除手段2：进攻目标躯干。
...
* 效果：目标获得（1+检定差值/15四舍五入）点只可用于移动的额外行动点（最多4点），以及一次额外的进入子弹时间的机会；本状态不可叠加，但可以取较好结果者替代
* 解除手段1：1行动轮后自动解除。
* 解除手段2：与<a href="#石附" target="_blank">“石附”</a>状态相互抵消。
* 弱化版本：降低一个阶级
* 词条：内生型、可升级
...
* 解除手段1：2行动轮后自动解除。
* 解除手段2：受到来自劝服者所在的一方的伤害解除。
* 弱化版本：效果变为：立刻扣除4点行动点。
* 词条：话术

### <h id="巧言善辩">巧言善辩</h>

* 效果：你感知范围内的某角色受到话术影响时，允许免行动点用话术与施加话术影响的角色进行对抗，如果你成功，你对施加话术影响的角色施加你的话术效果，若失败，则施加话术影响的角色的话术照常生效
* 解除手段1：准备时，进攻施法者头部。
* 解除手段2：进攻目标头部。
* 弱化版本：弱点变为身上的任何部位。
* 词条：信仰
...
* 效果：被施加此状态时，由状态施加者选择你的2个属性值，将这两个属性值交换
* 解除手段1：准备时，进攻施法者惯用手。
* 解除手段2：进攻目标惯用手。
* 弱化版本：弱点变为身上的任何部位。
* 词条：信仰
...
* 效果：角色获得飞行的能力；原先就能飞行的，在空中停留不再因护甲减少行动点恢复量
* 解除手段1：准备时，进攻施法者左右脚。
* 解除手段2：进攻目标左右脚。
* 弱化版本：弱点变为身上的任何部位。
* 词条：信仰
...
* 效果：应对施法者一方的话术效果时，对抗检定获得一枚惩罚骰。
* 解除手段：1行动轮之后自动解除
* 弱化版本：新增解除手段：被动对抗一次话术效果后解除
* 词条：话术
...
* 效果：角色搬运、攀爬检定获得一枚奖励骰，且搬运检定提供的差值+0.5倍，通过搬运投出造成伤害时，计算武器威力+2
* 解除手段1：准备时，进攻施法者左右手。
* 解除手段2：进攻目标左右手。
...
* 效果：叠加1层<a href="../mark/#研习印记" target="_blank">“研习印记”</a>。
* 解除手段：1行动轮后自动清空
* 弱化版本：叠加层数减半（向下取整）。
* 词条：外生型
...
* 效果：记录你被附加本状态时的位置，之后该状态的施加者可随时消耗1行动点让你强行返回这个位置；如果该状态的施加者是你的队友，你自己也可以随时消耗1行动点返回这个位置
* 解除手段1：准备时，进攻施法者左右脚。
* 解除手段2：进攻目标左右脚。
* 接触手段3：效果生效后自动解除。
