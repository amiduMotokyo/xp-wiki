...
* 效果：这件装备无法通过外力卸除，若为防具，额外生命归零时，延后到下回合再损坏
...
* 效果：占地长宽各+1，所有装备跟着一起变大，体质与力量值+25％，获得一层威压效果。
* 解除手段：1回合后自动解除。
* 弱化版本：体质与力量属性增长效果失效
* 词条：外生型
...
* 被捆绑方行动轮内只能执行以下行动：1、使用缴械对抗捆绑方的抵抗检定（或是造成捆绑状态的检定）来摆脱捆绑状态（一次2行动点，缴械没过可以继续使用）；2、执行其他不需要身体动作的行动
* 解除手段1：捆绑方主动解除。
* 解除手段2：捆绑方若受到伤害，则过抵抗（或是造成捆绑状态的检定），若没过，则强制解除捆绑状态。
...
* 效果：若处于水中，或是处于<a href="#潮湿" target="_blank">“潮湿”</a>状态，则无视<a href="#潮湿" target="_blank">“潮湿”</a>状态的负面效果，每次行动轮开始时获得一个生命值为5的<a href="#水盾" target="_blank">“水盾”</a>，若下次行动轮开始时本次的水盾未被击破，则回复5点HP与3点SP。
* 解除手段1：准备时，进攻施法者躯干。
* 解除手段2：进攻目标躯干。
...
* 效果：每次行动轮开始时，以下效果三选一：获得8点点数，可用于恢复体力或精神力；弱化自身的一个指定的内生型或外生型异常状态效果；将一种内生型或外生型印记层数清除一半（至少清除1层）
...
* 解除手段：当<a href="../../status/normal/#昏睡" target="_blank">“昏睡”</a>状态即将接触时，代替<a href="../../status/normal/#昏睡" target="_blank">“昏睡”</a>状态被解除。
* 弱化版本：无
* 词条：内生型
...
* 效果：角色搬运检定获得一枚奖励骰，且搬运检定提供的差值+0.5倍，通过搬运投出造成伤害时，计算武器威力+2
* 解除手段1：准备时，进攻施法者左右手。
* 解除手段2：进攻目标左右手。
...
* 解除手段：在自己的行动轮内花费2行动点，或者随时花费一次子弹时间的机会，解除一个武器或防具的松懈状态
* 弱化版本：直接解除本异常状态
* 词条：外生型
