...
* 效果：处于这种状态下的角色可以分辨出眼前素材的本质，使得他们可以用“侦查”检定来代替特殊技艺“采集”。<br>允许在战斗中花费所有基础行动点来进行长休中的“采集素材”行动。（每一次战斗最多采集总共+6素材）
* 解除手段：1回合后自动解除。
...
* 效果：友方单位只要处于施法者的感知范围内，每次成功的进攻都可以对目标敌人附加一层<a href="../mark/#损伤印记" target="_blank">“损伤印记”</a>。
* 解除手段：进攻施法者左右手。
* 弱化版本：本效果起作用的感知范围减半。
* 词条：信仰
...
* 解除手段3：被捆绑方的行动轮内，使用缴械对抗捆绑方的抵抗检定来摆脱捆绑状态（一次2行动点，缴械没过可以继续使用）。
* 弱化版本：直接解除本异常状态
* 词条：施受绑定
...
* 效果：目标每回合开始时进行幸运检定（100-jrrp），若失败，目标这回合的下一个成功的检定自动失败。
* 解除手段1：准备时，进攻施法者躯干。
* 解除手段2：进攻目标躯干。
...
* 效果：所有装备的力量阈值+10（若因此超过自身力量，若是武器则自动掉落，若是任何一件防具则无法移动）。受到的<font color="#dddd00">雷电伤害</font>减半。免疫<a href="../mark/#静电印记" target="_blank">“静电印记”</a>与“静电场”效果。
...
* 解除手段1：受击解除（包括缴械）。
* 解除手段2：花费6点基础行动点解除。
* 弱化版本：“失去意识”词条失效，效果改为：立刻将`行动顺序表中的`敏捷值减少5点，直到轮到目标下一次行动轮为止。
* 词条：外生型、失去意识
