...
扰空螺号|可以使用5次，每行动轮限定一次，吹响号角发出干扰飞行的巨响，使得10范围内（无视垂直距离）的所有空中角色立刻结算一次在空中结束回合的惩罚（行动力回复减少除外）|1|1000
捕空网|需要通过投掷或发射器检定与空中目标的防守检定进行对抗，保底将其高度降低1格，若成功，额外降低（差值因子）格，计算距离时额外+4|玩家数量|200
