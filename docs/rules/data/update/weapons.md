...
:--:|:--:|:--:|:--:|:--:|:--:
5|蓄力箭|特殊战技|与武器一致|5|消耗3点行动点，进行一次检定值获得-4奖励，且总伤倍率+0.5的射击，只能用于直射
15|弓弦勒击|特殊战技|与武器一致|8|快速移动到范围1内的敌人正后方进行攻击，若成功，造成伤害的同时，对手进行抵抗判<a href="../../status/normal/#晕眩" target="_blank">“晕眩”</a>
25|弓斗术|战技|被动|0|每行动轮限1次，每用弓系武器消灭一个攻击范围/2内的敌人时，若攻击范围/2内还有其他敌人，则行动点恢复一半，且恢复进攻阈值衰减
...
15|连射|战技|被动|0|本行动轮内，装弹数、弹药量和行动点足够的情况下，可选取当前射击的检定结果作为之后几箭的共同结果
...
熟练度等级|名称|战技/特殊战技|类型/挂钩属性|消耗SP|效果
:--:|:--:|:--:|:--:|:--:|:--:
5|规范动作|战技|被动|0|投掷物件和使用飞镖系武器时无视近战接触惩罚<br>若你获得了8级的投掷系武器，则你每有6点经历点（不算补偿），该武器等级+1（不影响售出价格，增加的等级无法印刻印痕），最多加到15级
15|多重投掷|战技|被动|0|射击时，可以选择以消耗2发同种弹药，射程减半、武器威力减半二选一为代价，让本次进攻的攻击段数+1
25|流星雨|战技|被动|0|对于同一目标，每进行一次射击（包括攻击段数），造成的伤害+1，最多叠加5次<br>若目标进行了移动或者你主动改变了进攻目标，则本效果刷新
...
25|一炮双响|战技|触发|5|一行动轮一次，快速装载两种弹药（可以是同类型）进行定点投射，一次检定全部生效（相当于第二次的检定应用第一次的检定成果）

### 火器系

熟练度等级|名称|战技/特殊战技|类型/挂钩属性|消耗SP|效果
:--:|:--:|:--:|:--:|:--:|:--:
5|专注装弹|战技|被动|0|如果本行动轮没有移动过，则装弹操作不再视为一个<a href="../../../V4.x rules/Extra/BulletTime" target="_blank">破绽</a>
15|机动装弹|战技|被动|0|允许边移动边装弹，将移动消耗的行动点等量适用于装弹消耗的行动点
25|抵近射击|战技|被动|0|你的行动轮外，当敌方进入（或出现在）你攻击射程/2范围时，视为对你个人露出一次破绽，若你进入子弹时间进攻（也只能用火器进攻），则获得一枚奖励骰，且本次攻击无视近战接触惩罚
...
15|绕后闪现|特殊战技|敏捷|5|对4格范围内的目标使用，如果可以，直接瞬移到目标的正后方1格；之后若立刻发动攻击，对方目标视为<a href="/rules/V4.x rules/12.properName/#措手不及" target="_blank">措手不及</a>；之后若立刻远离目标，目标无法借机
...
15|剑刃风暴|特殊战技|与武器一致|10|对攻击范围内的所有敌人发动攻击，且可以一边攻击一边移动最多3格<br>若目标敌人要借机，则只能通过拼刀来借机，用武器检定来对抗技能使用者的剑刃风暴技能，且技能使用者有一枚奖励骰
...
熟练度等级|名称|战技/特殊战技|类型/挂钩属性|消耗SP|效果
:--:|:--:|:--:|:--:|:--:|:--:
5|锐锋突击|战技|被动|0|冲锋过程中，每多移动1格，额外造成的伤害+1，最多可以获得10点额外伤害<br>若为骑乘状态，移动格数从进入疾驰状态开始统计，造成伤害后重新统计
15|精准突刺|战技|被动|0|攻击和长枪的所有技能都获得穿透效果（允许对攻击范围内一直线上的所有敌人造成伤害）<br>如果长枪的一次进攻只命中一个目标，则可以无条件瞄准部位
25|撑杆高跳|特殊战技|与武器一致|15|先进行跳跃检定（不计入跳跃次数），最大水平与垂直位移与跳过的格数相同，随后战技对抗，若成功则从高处跃下把敌人扎倒在地（强制<a href="../../status/normal/#倒地" target="_blank">“倒地”</a>），同时附加每1高度2点固伤
...
15|下段突刺|特殊战技|与武器一致|8|可以攻击武器范围内一直线上的所有敌人；处于长距离武器惩罚范围外的敌人受到伤害的总伤倍率+0.5
25|无方扫|特殊战技|与武器一致|15|攻击武器范围内的所有敌人，处于长距离武器惩罚范围外的敌人受到的总伤倍率+1倍；其他敌人若对抗失败，则强制<a href="../../status/normal/#倒地" target="_blank">“倒地”</a>

### 长戟系

熟练度等级|名称|战技/特殊战技|类型/挂钩属性|消耗SP|效果
:--:|:--:|:--:|:--:|:--:|:--:
5|失衡打击|战技|触发|3|每次进攻成功后可选择触发：让对方抵抗判<a href="../../status/normal/#失衡" target="_blank">“失衡”</a>，若对方已经陷入<a href="../../status/normal/#倒地" target="_blank">“倒地”</a>状态，则造成伤害倍率+0.2
15|扰乱威胁|特殊战技|与武器一致|5|若使用本战技造成伤害，目标抵抗判<a href="../../status/normal/#扰乱" target="_blank">“扰乱”</a>状态
...
15|卸力|特殊战技|与武器一致|5|本特殊战技不造成HP伤害，而是造成SP伤害，且总伤倍率+0.5
25|刚柔共济|战技|被动|0|所有棍系武器可造成1.0倍的SP伤害，每行动轮第一次攻击不算行动点，不算阈值衰减

### 镰刀系

熟练度等级|名称|战技/特殊战技|类型/挂钩属性|消耗SP|效果
:--:|:--:|:--:|:--:|:--:|:--:
5|勾魂|特殊战技|与武器一致|5|此技能攻击范围+1，暴击阈值+5，若对抗成功，可选择将敌人拉近自己若干格（最近到自己身前），自己也可以选择是否后退最多相同的距离
15|穿心刺|战技|触发|3|进攻时选择触发，锁定进攻敌人的躯干，可将暴击效果改为额外造成对方HP上限50%（boss则为25％）的伤害，每个敌人仅生效一次，击败小兵类敌人会返还行动点，触发暴击效果后本行动轮无法再次使用
25|夺魂|特殊战技|与武器一致|10|瞬移到最远4范围内的敌人身后最远武器范围内发起攻击，敌人应对本次攻击视为<a href="/rules/V4.x rules/12.properName/#措手不及" target="_blank">措手不及</a>，锁定头部，造成伤害倍率+0.5
...
15|蓄力重击|特殊战技|与武器一致|10|对前方第二格区域发动震地冲击，对该格及其周围一圈的敌人造成伤害，且该格内的敌人受到伤害的总伤倍率+0.5
25|大风车|特殊战技|与武器一致|5|本技能可以连续使用，断连后消耗本行动轮的使用次数<br>一边挥舞武器一边前进，对攻击范围内的所有敌人造成伤害，每使用一次可选择前进1格，可以推着敌人一起前进
...
:--:|:--:|:--:|:--:|:--:|:--:
5|魔法飞弹|战技|被动|0|在“固定模式”下，你可以将法杖射程值改成施法范围，普通攻击视为远程的魔法攻击（应用魔法对抗流程）
15|延展施法|战技|被动|0|在“固定模式”下，杖的施法范围+1，每经过1行动轮额外+1，最多增加至施法范围面板数值的1.5倍<br>当你切换至“携带模式”时，施法范围恢复至面板数值
25|焦点固定|战技|被动|0|在“固定模式”下，使用杖进攻或施法时（仪式之法则为被打断需要魔抗检定时），阈值+4；经过1行动轮后，变为阈值+8；经过2行动轮后，变为阈值+8，检定获得一枚奖励骰<br>当你切换至“携带模式”时，此效果重新开始累计

### 魔装系

熟练度等级|名称|类别|类型/挂钩属性|消耗SP|效果
:--:|:--:|:--:|:--:|:--:|:--:
5|近接释能|战技|被动|0|对近战范围内的单位进行攻击或施法时（仪式之法则为被打断需要魔抗检定时）阈值+8
...
25|能级超载|战技|被动|0|自身回合内每进行2次施法，获得1层<a href="../../status/mark/#能缰印记" target="_blank">“能缰印记”</a>

### 典籍系

熟练度等级|名称|类别|类型/挂钩属性|消耗SP|效果
:--:|:--:|:--:|:--:|:--:|:--:
5|寻求新知|战技|被动|0|自身回合内每进行一次施法，典籍攻击范围+1，至多增加至与施法范围相同；<br>回合结束时，若本回合内典籍攻击范围未增加过，则将其攻击范围重设为初始范围；<br>购物时，作为购买魔法书的代替，可以以每个初级/中级/上级魔法200/300/400的费用记载之
15|灵光速记|战技|触发|4|施法检定掷出困难成功及以上时（仪式之法则改成准备完成时）可选择消耗1AP触发，记载该魔法；<br>斗法成功时可选择消耗2AP触发，记载发起方释放的魔法并露出一个破绽<br>（对于某些未出现在wiki上的魔法，主持人可酌情调整该条效果）
25|禁忌索引|战技|触发|2|施放记载中的魔法时（仪式之法则为被打断需要魔抗检定时），若已习得之则可选择触发，当次施法阈值+16，检定值-8，额外获得一枚奖励骰，当次释放结算完成后，解除该魔法的记载状态
...
25|二次输魔|战技|被动|0|纸符进攻或施法时（仪式之法则为被打断需要魔抗检定时），可以额外消耗一枚纸符，使当次检定阈值+8，检定值-4
