...
!!! success "战术技能的升级"
    每种战术都可以进行精进，顺序为：原始→初级→中级→上级，不可跳级精进。
...
* 你与“本轮选定共享的那名队友”作为一组，每行动轮最多触发1次夹击破绽。
...
当背靠背叠到多层时，大于1层的部分获得的战术奖励减半，且不应用战术技能的升级奖励。
...
形态|效果描述
:--:|:--:
原始|到下行动轮你的行动开始为止，你对抗远程攻击的检定，以及对抗近战攻击的闪避检定附加战术奖励
初级|你对抗直射时闪避阈值不再减半，对抗火器时的防御阈值也不再减半
中级|当你对抗物理攻击的闪避检定失败时，也可以如同防御检定失败一般减少25％的伤害
...
初级|到下行动轮你的行动开始为止，你获得1范围的<a href="/rules/V4.x rules/12.properName/#代防" target="_blank">代防</a>，且本轮第一次代防免消耗
...
形态|效果描述
:--:|:--:
原始|到下行动轮你的行动开始为止，受到对方攻击后，立刻获得一次免行动点的普通攻击机会（算衰减），且本次进攻阈值附加战术奖励，每行动轮一次
