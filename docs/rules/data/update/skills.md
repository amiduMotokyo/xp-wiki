...
30|重磅炸弹|战技|被动|0|现在你可以将你搬运的角色投出去，规则与举重若轻技能中的类似，区别在于：会对投出的角色造成（重物搬运成功难度值/5+投出搬运检定差值/10）点固伤，若目标只是普通地块，则补过一次普通的搬运检定<br>利用环境物对敌人造成伤害时（包括举重若轻和重磅炸弹），计算武器威力×1.5
