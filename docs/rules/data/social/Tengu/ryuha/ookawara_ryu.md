...
十文字枪法|战技|被动|0|允许使用十文字枪法·进攻
惊骇坐骑|战技|被动|0|若你使用带有“十文字标签”的武器进攻敌方坐骑，且进攻成功，对方抵抗判<a href="../../../../status/normal/#倒地" target="_blank">“倒地”</a>；若本次进攻本身就有倒地效果，则对方抵抗阈值减半
就势|战技|被动|0|若你的某一十文字枪法进攻成功，你可以选择你已经随机到的剩余某一十文字枪法，令其触发后附加以下效果三选一：回复6点SP；进攻造成伤害+0.5倍；本回合子弹时间机会+1次（不可自叠）
流转|战技|被动|0|在你的行动轮外，若敌人恰好满足十文字枪法·进攻中“拔突”、“水车”、“斩落”中的行为条件时，且你恰好随机到了对应枪法，则该敌人露出对你个人而言的破绽，在此次破绽中，你可以也只能对其使用对应的枪法（SP不够则无效）
...
序号|名称|SP消耗|效果
:--:|:--:|:--:|:--:
1|拔突|6|敌人最近一次进行的行动为“防御”时，你攻击前可以选择触发：本次攻击附加战术奖励
2|卷落|4|本次进攻敌人只能使用“防御”来应对
3|水车|6|敌人最近一次进行的行动为“闪避”时，你攻击前可以选择触发：本次攻击附加战术奖励
4|粘花|4|本次进攻敌人只能使用“闪避”来应对
5|斩落|6|敌人最近一次进行的行动不为“闪避”“防御”或“进攻”时，你攻击前可以选择触发：本次攻击附加战术奖励
...
序号|名称|SP消耗|效果
:--:|:--:|:--:|:--:
1|引落|6|敌人向你发起物理进攻时可以选择触发：无效化本次进攻（若条件允许，可以连携触发柄返）
2|柄返|4|敌人向你发起进攻，且敌人处于你的进攻范围之内时，可以选择触发：对方攻击结束后立刻进行一次免行动点不计衰减的普通攻击机会，且此次普通攻击允许触发十文字进攻枪法
3|飞鸟|6|敌人移动到你的进攻范围之内时，可以选择触发：立刻进行一次免行动点不计衰减的普通攻击机会，且此次普通攻击允许触发十文字进攻枪法
4|借势|0|受到伤害时（自己行动轮内外皆可）可以选择触发：回复与受到的伤害（不包含对护甲的）等值的SP
5|擦入|6|与敌人拼刀前（自己行动轮内外皆可）可选择触发：本次拼刀过程中，若你拼刀成功，造成（武器威力/4四舍五入）点额外伤害；若你对抗失败，也可按照对抗成功的规则对对方造成伤害