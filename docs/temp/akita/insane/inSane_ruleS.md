...
* 《》符号：作为特别的符号，特指特技。如《听觉》。特别地，如果写作《听觉/知觉7》，其意思即为“这是位于 知觉领域的 第七个特技，听觉”。详细见特技说明部分。
...
编号|特技名|描述
:--:|:--:|:--:
1|焚烧|操控火或者热量的能力，这意味着你对各类易燃物和爆炸物非常熟悉。
2|拷问|使他人痛苦以达成自己目的，强迫他人做你指定的行为。
3|束缚|使用如绳索这样的道具限制他人自由。
4|威胁|抓住对方的痛处，用暴力手段或是你的背景设定进行恐吓。
5|破坏|将物件或生物体破坏。*你是芙兰朵露*
6|殴打|殴打和圈踢，差不多就是小混混打架的程度吧。似乎也不必然是赤手空拳。
7|切割|使用刀具切割物品或生物体的能力，特别指出可以“切断”。*切肉很难啊就是说*
8|穿刺|将物品的前端刺入某物的能力（？）。
9|射击|使用枪械和弓箭等远程武器的能力。*我怀疑对回旋镖也适用。*
10|战争|适用于大规模战场的技能，意味着你可以指挥一支军队（或者类似的东西）。
11|埋葬|将东西（意味深）埋进土里或沉进水底的能力。
...
另外，在表格中可以发现，前文提到的六大领域特技表中，特技名的两边有着意味不明的空白格子，我们将这些空白格子称为【gap】。
...
每个pc在游戏开始时都拥有6点【HP】，同时也拥有6的HP上限。部分特特技提高这个上限。
...
每个pc在游戏开始时都拥有6点【SAN值】，同时也拥有6的SAN值上限。但是每得到一个怪异领域的特技，就会扣除1点【SAN值】，同时扣除1点上限。部分特特技提高这个上限。

### 9.主动技能（异能）

每个pc都可以在特技库中选择两项特技。此外【基本攻击】和【战场移动】是每个pc都有的特技，不需要选择。

**选择主动技能时，我想大家应该会比较重视其作用，而不是像我这样只注重角色的扮演，所以我特地去找了下特技描述中提到的各种检定、名词。但是要解释这些名词又不得不把整套系统都搬出来……所以，大家别急，等我把系统全都翻译完之后你们就能看懂描述了。车卡时先跳过这里也没关系**

主动技能分为攻击技能、支援技能和装备技能三种。

* 攻击技能：战斗时，在自己的回合使用的特技，可以给予目标伤害。
* 支援技能：给冒险提供各种支援的特技，这些特技可能需要启动条件，不限于自己的回合生效。
* 装备技能：=被动，获得后常时生效。

主动技能与特技挂钩，使用主动技能时需要用指定的特技进行检定。但是与xp团不同地，挂钩的特技可以由pl指定，而且相当自由。比如，【基本攻击】的挂钩特技可以选择六大领域的任何一个特技。

但是我没有看到有说明这个挂钩特技是否可以随时变化的说明。我觉得应该不行。默认为在车卡阶段获得特技后，指定一个挂钩特技，接着这个挂钩特技就不可更改。
...
大闹|领域指定：暴力|按照你已经公开的【疯狂】总数选择等量的目标进行命中检定。（如果使用这个特技的是怪物或npc，则取1d6个目标）命中检定只需要进行一次，成功后，所有被选中的目标进行回避检定。回避检定失败时，给予目标1d6-2点HP伤害（最低0点）。|能一次攻击大量敌人。
狙击|领域指定：技术/《射击》|选择一个杂兵目标，进行命中检定。命中检定成功时，给予对方2d6的HP伤害。（需注意这个特技没有提对方的回避检定，可能是必中特技）|对特定的敌人进行狙击。
...
魅惑|领域指定：情感|感情检定成功时可以使用。感情检定的目标以这个特技的挂钩特技进行检定。如果对方检定失败，则pc可以自由指定这个目标对自己的【感情】之种类、属性。而且可以抢夺一个目标的【住所】或者物品。|可怜的汤姆，被玩弄在股掌之中。
...
闪电战|领域指定：技术|pc获得他人的【住所】时可以使用。挂钩特技检定成功时，可以对该【住所】对应的人发起一场战斗。|憋不住笑了草。
报复|领域指定：知识|pc受伤害时可以使用。选择造成伤害的家伙作为目标，挂钩特技成功时，给予对方1点HP伤害。对方可以进行你所进行的同样特技鉴定进行抵抗，对方成功的话无效这个效果。|反手一巴掌。
开发|领域指定：知识/《魔术》|pc是该场景的主角时可以使用。消耗1点【SAN值】或者【HP】。挂钩特技检定成功时，获得1个道具。|俺寻思这玩意能用。
研究|领域指定：知识|pc是该场景的主角时可以使用。选择一个【秘密】被pc掌握的角色，挂钩特技检定成功时获知目标的【住所】。|开开你的
许愿|可变指定：怪异|主阶段的，场景与场景之间的间隙时可以使用。在怪异领域中随机指定一个特技进行检定。检定成功时，使用者的【SAN值】减少1点，可以再进行一次剧情场景。这个场景中的所有判定获得目标值-1的修正。|你许下愿望，但是为你实现愿望的是谁？
...
机智|可以在六大领域中再学一个特技，这个特技最多学两次。|学什么都很快
资产|追加获得2个道具。|实现了财富自由
...
3. 【确认目标值】
在INSANE规则中，你需要投出比目标值大的数目才算成功。所以，我们在检定之前需要确认这次检定的目标值。
