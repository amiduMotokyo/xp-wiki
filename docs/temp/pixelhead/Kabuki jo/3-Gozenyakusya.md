...
继承旗手职业特性，执行“幕”时，可以以一种旗手光环本回合中失效为代价，执行强化版本的“幕”；战斗胜利时，每成功执行两次“幕”，则获得月零用20%的金钱
...
谎言成真之刻|战技|触发|10|至每回合只能使用一次，目标所有需要消耗AP的非移动行动若不使用常态乔装行动则失效，常态乔装行动对抗失败时新行动失效，常态乔装行动成功时可消耗1AP，免AP使用原来的行动且不会露出破绽
